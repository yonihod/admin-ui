...
const Space: React.FC<{ id: string }> = ({ id }) => {
    const { data: space } = trpc.useQuery(["spaces.get-space-by-id", { id }]);
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <Tuples label="Created By:" value={created_by.name} classes={["capitalize"]}/>
                {modified && <Tuples label="Modified At:" value={modified}/>}
                {modified_by && <Tuples label="Modified By:" value={modified_by?.name} classes={["capitalize"]} />}
            </div>
        </div>
    );
}
